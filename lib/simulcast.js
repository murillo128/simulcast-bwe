...
const Capabilities = {
	video : {
		...
		extensions	: [ "urn:3gpp:video-orientation","http://www.ietf.org/id/draft-holmer-rmcat-transport-wide-cc-extensions-01","urn:ietf:params:rtp-hdrext:sdes:rtp-stream-id"],
		simulcast	: true
	}
};
	...
	connection.on('message', (frame) =>
	{
		...
		if (msg.cmd==="OFFER")
		{
			...
			for (let offered of offer.getStreams().values())
			{
				...
				//Start at min layer
				connection.transponder.selectEncoding("c");
